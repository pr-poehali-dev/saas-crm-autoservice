...
export default function PriceModule() {
  ...
  const reset = () => {
    ...
    setError(""); setSupplierFilter(""); setExpanded(null);
  };
  ...
  const uniqueBrands = brands.reduce<BrandOption[]>((acc, b) => {
    if (!acc.find((a) => a.name.toLowerCase() === b.name.toLowerCase())) acc.push(b);
    return acc;
  }, []);
  ...
  return (
    <div>
            ...
            {selectedBrand && (
              <>
                ...
                {step === "results" && brands.length > 1 && (
                  <button onClick={() => { setStep("brands"); setParts([]); setError(""); setSupplierFilter(""); }}
      ...
      {step === "brands" && !loading && uniqueBrands.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {uniqueBrands.map((b, i) => {
              const sups = [...new Set(brands.filter((br) => br.name.toLowerCase() === b.name.toLowerCase()).map((br) => br.supplier))];
              return (
      ...
      {step === "results" && !loading && filtered.length > 0 && (
        <>
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            <span className="text-sm text-muted-foreground">
              <strong className="text-foreground">{filtered.length}</strong> позиций, <strong className="text-foreground">{totalStocks}</strong> предложений
            </span>
            {suppliers.length > 1 && (
              <div className="flex gap-1.5 ml-auto">
                <button onClick={() => setSupplierFilter("")}
                  ...
                  style={!supplierFilter ? { background: "hsl(var(--primary))" } : {}}>Все ({parts.length})</button>
                {suppliers.map((s) => {
      ...
      {step === "results" && !loading && parts.length === 0 && !error && (
